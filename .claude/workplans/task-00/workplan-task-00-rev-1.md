...
**Dependency Management**: Proyecto migrado a **uv** como package manager oficial. Todos los comandos usan `uv run` para asegurar consistencia con virtualenv y lockfile (`uv.lock`).
...
- [ ] **1.2** Virtual environment activado (uv automático)
...
**5. uv sync falla con dependencias conflictivas**:
- **Solución**: Revert a pip install -e . (fallback en CLAUDE.md)
- **Trade-off**: Perder lockfile uv.lock (aceptable para desarrollo local)
...
**Decisión TOMADA**: **uv** como package manager oficial
...
**Trade-off aceptado**: uv es relativamente nuevo (1.0 en 2024) → Fallback a pip documentado en CLAUDE.md
...
**FIN DEL WORKPLAN - TASK-00 REV-1**
