# Workplan: Fase 7 - RAG Question Answering
...
- `tests/test_rag_retriever.py` - **NUEVO** tests unitarios
- `tests/test_query_tool.py` - **NUEVO** tests de integración
...
- Sistema retorna respuesta coherente con citas académicas
...
5. **1 test de integración end-to-end** - Validar pipeline completo
...
| `tests/test_rag_retriever.py` | Unit tests | Creación |
| `tests/test_query_tool.py` | Integration tests | Creación |
...
| **T7.4** | Implementar pipeline en `RAGRetriever.answer_question()` | 🔴 CORE | - Embed question con EmbeddingService<br>- Query VectorStore con embedding<br>- Build context from top-k chunks<br>- Generate answer con OpenAIClient<br>- Format QueryResult con sources | T7.3 | 1.5h |
| **T7.5** | Crear `tools/query.py` con tool `query_papers` | 🔴 CORE | - Decorator `@mcp.tool()`<br>- Parámetros: question, session_id?, top_k=5<br>- Instancia RAGRetriever<br>- Return QueryResult.model_dump() | T7.4 | 1h |
...
| **T7.7** | Unit test para `generate_answer()` con mock | 🔴 CORE | - Mock OpenAI API response<br>- Verificar prompt construction<br>- Verificar retry logic | T7.1 | 45m |
| **T7.8** | Integration test end-to-end | 🔴 CORE | - Pipeline completo: search→download→index→query<br>- Verificar QueryResult tiene answer y sources<br>- Verificar sources tienen metadata correcta | T7.5, T7.6 | 1.5h |
...
| **T7.10** | Tests comprehensivos para retriever | 🟡 Enhanced | - Test edge cases (no results, empty context)<br>- Test con múltiples sessions<br>- Validate similarity score conversion | T7.4 | 1h |
...
6. T7.10 (Enhanced) - Tests comprehensivos

**Fase 3: MCP Tools (2h)**
7. T7.5 - Tool `query_papers`
8. T7.6 - Tool `list_sessions`

**Fase 4: Validación (1.5h)**
9. T7.8 - Integration test end-to-end

**Fase 5: Enhanced (1h - opcional)**
10. T7.9 - Cross-session query
...
2. **Integration Tests** - Componentes reales en pipeline end-to-end
3. **Manual Smoke Tests** - Validación con MCP Inspector y Claude Code

**Herramientas:**
- pytest para automatización
- pytest-asyncio para async tests
- unittest.mock para mocking OpenAI API
- fixtures reales de Fase 6 (sessions con PDFs indexados)

### 5.2 Test Cases Principales

**Unit Tests (`tests/test_rag_retriever.py`):**

| Test Case | Descripción | Aserciones |
|-----------|-------------|------------|
| `test_generate_answer_success` | Mock OpenAI API retorna respuesta | - Prompt contiene question y context<br>- Retry logic funciona<br>- Tokens logged |
| `test_generate_answer_auth_error` | API key inválida | - AuthenticationError se propaga<br>- No retry para auth errors |
| `test_retriever_pipeline` | Mock embedder + vectorstore + openai | - Embedding se genera<br>- Query ejecuta<br>- Context building correcto<br>- QueryResult válido |
| `test_retriever_no_results` | VectorStore retorna lista vacía | - Return QueryResult con "No relevant information found" |
| `test_similarity_conversion` | Distance → similarity score | - `similarity = 1.0 - distance`<br>- Range [0, 1] |

**Integration Tests (`tests/test_query_tool.py`):**

| Test Case | Descripción | Aserciones |
|-----------|-------------|------------|
| `test_query_papers_end_to_end` | Pipeline completo con session real | - Pregunta genera respuesta coherente<br>- Sources contienen metadata<br>- Citas presentes en answer |
| `test_list_sessions` | Tool retorna sessions | - List no vacía<br>- Cada session tiene metadata keys |
| `test_query_invalid_session` | session_id no existe | - Raise error apropiado |
| `test_query_not_indexed` | Session existe pero no indexada | - Raise error o return mensaje |
...
2. **Tool `query_papers` - Query básica**
   ...
   - [ ] Sources contiene 3 chunks (o menos si no hay suficientes)
   - [ ] Metadata incluye: paper_title, authors, year

3. **Tool `query_papers` - Sin session_id (Enhanced)**
   - [ ] Ejecutar con: `{"question": "What are transformers?", "top_k": 5}`
   - [ ] Busca en todas las sesiones
   - [ ] Retorna resultados merged

4. **Tool `query_papers` - Sin resultados**
   - [ ] Ejecutar con pregunta irrelevante: `{"question": "How to cook pasta?", ...}`
   - [ ] Retorna mensaje "No relevant information found" o similar

5. **Validación de Citas**
   - [ ] Respuesta incluye referencias académicas (ej: "Vaswani et al., 2017")
   - [ ] Sources en QueryResult coinciden con papers citados
...
### 5.4 Plan de Tests en `tests/`

**Archivo:** `tests/test_rag_phase7.py` (NUEVO)

**Contenido mínimo:**
- Test `generate_answer()` con mock OpenAI API
- Test `RAGRetriever.answer_question()` con mocks
- Test pipeline completo end-to-end (requiere session fixture)
- Test edge cases (no results, invalid session)
...
- **Mitigación:**
  1. Usar mocks para todos los tests (no llamadas reales)
  2. Si necesario real test, usar delays entre calls
  3. Upgrade a tier con mayor límite (si crítico)
...
- **Señal:** Queries toman >5s para top-k=5
...
**NO-GO si:**
- ❌ OpenAI API authentication falla (revisar API key)
- ❌ ChromaDB query retorna errores (revisar Fase 6 indexing)
...
**Si falta 30% tiempo:**
1. ❌ Recortar Enhanced: cross-session query (T7.9)
2. ❌ Recortar tests comprehensivos (T7.10)
3. ✅ Mantener: Core MVP completo

**Si falta 50% tiempo:**
1. ❌ Recortar todo Enhanced
2. ❌ Simplificar tests: solo 1 integration test (T7.8)
3. ❌ Skip unit test para `generate_answer()` - confiar en integration test
4. ✅ Mantener: `generate_answer()`, `RAGRetriever`, 2 tools
...
3. ✅ **Testing:**
   - Al menos 1 integration test end-to-end pasa
   - Unit tests para `generate_answer()` pasan (o skip si falta tiempo)
   - Smoke test manual ejecutado (checklist completado)

4. ✅ **Calidad de Outputs:**
   - Respuestas son coherentes (no gibberish)
   - Citas académicas presentes en respuestas
   - Sources en QueryResult tienen metadata correcta

5. ✅ **Documentación:**
   - README actualizado (si aplica - probablemente no para Fase 7 interna)
...
- Citas pueden no ser 100% precisas (LLM puede alucinar autores)
- Performance de query depende de tamaño de collections (mitigado con max_chunks limit)

---

**Workplan persistido:** `.claude/workplans/task-08/workplan-task-08.md`
