# Workplan: Fase 4 - PDF Download (Revisión 4)
...
| 1.3 (rev-3) | 2025-12-29 | Mejoras aplicadas post-revisión con codex | Incorporar sugerencias de análisis automatizado: idempotencia, settings, criterios de aceptación, riesgos | Codex + Usuario |
| 1.4 (rev-4) | 2025-12-29 | Refinamiento de manejo de errores y validación | Aclarar errores en download_batch, comportamiento paper_indices, tests idempotencia, normalización pdf_path | Codex + Usuario |
...
assert len(result["download_metadata"]) == result["downloaded"]  # [NUEVO REV-3] Consistencia 1:1
assert all(meta["pdf_path"] in result["pdf_paths"] for meta in result["download_metadata"])  # [NUEVO REV-3] Coherencia
# [NUEVO REV-3] Orden determinista (sorted by rank)
assert result["pdf_paths"] == sorted(result["pdf_paths"], key=lambda p: int(p.split("_")[-1].split(".")[0]))
```
...
5. **Estrategia de Idempotencia** ⭐ **[NUEVO REV-3]**:
   - ✅ **Comportamiento por defecto (MVP)**: Skip - si `{pdf_dir}/paper_{rank}.pdf` existe y es válido (>0 bytes, magic bytes %PDF), no re-descargar
   - ✅ Verificar existencia con `aiofiles.ospath.exists(pdf_path)`
   - ✅ Si existe, añadir a `pdf_paths` pero NO incrementar `downloaded` (solo `skipped`)
   - ✅ Re-download solo si archivo corrupto (validación falla) o explícitamente solicitado
   - ✅ `download_metadata` reflejará `status: "skipped"` para PDFs existentes
   - ⚠️ **Enhanced**: Parámetro `force_redownload: bool = False` en download_papers (futuro)

6. **Settings.max_concurrent_downloads** ⭐ **[NUEVO REV-3]**:
   - ✅ Definir en `src/sortgs_mcp/config.py` dentro de clase `Settings`
   - ✅ Campo: `max_concurrent_downloads: int = 5` (valor por defecto)
   - ✅ Documentación: "Maximum number of concurrent PDF downloads"
   - ✅ Validación Pydantic: `Field(ge=1, le=20)` (entre 1 y 20)
   - ✅ Usado en: `PDFDownloader.__init__(semaphore=asyncio.Semaphore(settings.max_concurrent_downloads))`
   - ✅ Test: Mockear settings con `max_concurrent_downloads=2` y verificar que solo 2 descargas corren en paralelo

7. **Config.pdf_download_dir**:
   - ✅ Property en Settings que retorna session-specific path
   - ✅ `sessions_dir / session_id / "pdfs"`

8. **PDFDownloadResult Model** (models.py): **[NUEVO REV-2]**
   - ✅ Campos: session_id, downloaded, failed, pdf_paths, failed_papers
   - ✅ **Campo nuevo**: `download_metadata: list[DownloadMetadata]`
   - ✅ **Campo nuevo REV-4**: `error: str | None = None` (para errores pre-gather)
   - ✅ Tipos explícitos, validación Pydantic
   - ✅ Ejemplo JSON schema documentado
   - ✅ **[NUEVO REV-4] Normalización de pdf_path**:
     - Todos los `pdf_path` deben ser rutas relativas al directorio raíz del proyecto
     - Ejemplo: `data/sessions/{session_id}/pdfs/paper_0.pdf`
     - NO absolutos: `/home/user/...` (rompe portabilidad)
     - Conversión: `Path(pdf_path).relative_to(project_root)` antes de añadir a resultado
     - Ventaja: Compatibilidad con RAG/ChromaDB sin hardcoded paths
     - Test: Verificar que ningún path empiece con `/` o contenga rutas absolutas

9. **DownloadMetadata Model** (models.py): **[NUEVO REV-2]**
...
10. **Tests básicos offline**:
   ...
   - ✅ **[NUEVO REV-4]** Test idempotencia: Descargar mismo paper 2 veces, verificar skip en segundo intento
     - Primera descarga: `result1["downloaded"] == 1, result1["skipped"] == 0`
     - Segunda descarga: `result2["downloaded"] == 0, result2["skipped"] == 1`
     - Verificar `download_metadata` contiene `status: "skipped"` en segundo intento
...
class PDFDownloadResult(BaseModel):
    ...
    error: str | None = Field(
        default=None,
        description="[NEW REV-4] Error message if batch operation failed before gather"
    )
...
#### 11. **[NUEVO REV-4]** Manejo de Errores en download_batch ✅

**Decisión:** Distinguir entre errores pre-gather y errores por tarea
**Justificación:**
- Errores pre-gather (ej: fallo al crear carpeta PDFs) deben ser reportados claramente
- Errores por tarea (ej: 404 en un PDF) ya están manejados en failed_papers
- Usuario necesita saber si falló todo el batch vs solo algunos PDFs

**Implementación:**

**Manejo de errores pre-gather:**
- Si ocurre excepción ANTES de `asyncio.gather()` (ej: fallo al crear carpeta de PDFs):
  - Capturar con try/except en download_batch
  - Retornar `PDFDownloadResult` con `downloaded=0, failed=0, error="<descripción>"`
  - Campo `error: str | None = None` en PDFDownloadResult (ya añadido en modelo)
- Si ocurre excepción DURANTE gather (errores por tarea):
  - Ya manejado: cada tarea reporta su fallo en failed_papers
  - `gather(return_exceptions=True)` asegura que otras tareas continúan

**Código ejemplo:**
```python
async def download_batch(self, papers: list[Paper], pdf_dir: Path) -> PDFDownloadResult:
    try:
        # Pre-gather setup
        pdf_dir.mkdir(parents=True, exist_ok=True)  # Could fail here
...
#### 12. **[NUEVO REV-4]** Validación de paper_indices en Tool MCP ✅

**Decisión:** Validar y sanitizar paper_indices antes de procesar
**Justificación:**
- Usuarios pueden pasar índices inválidos (fuera de rango, duplicados)
- Mejor experiencia: ignorar inválidos que fallar todo
- Contadores claros: solo contar papers únicos procesados

**Validación de paper_indices:**
- Si `paper_indices` contiene índices fuera de rango:
  - Ignorar índices inválidos (no error)
  - Log warning: "Skipping out-of-range index {idx}"
  - Solo procesar índices válidos [0, len(session.results)-1]
- Si `paper_indices` contiene duplicados:
  - Deduplicar automáticamente (usar set() → sorted list)
  - Descargar cada paper solo una vez
- Efecto en counters:
  - `downloaded`: Solo cuenta PDFs únicos exitosos
  - `failed`: Solo cuenta intentos únicos fallidos
  - `skipped`: Solo cuenta PDFs únicos ya existentes

**Código ejemplo en tool:**
```python
async def download_papers(
    ...
    if paper_indices is not None:
        valid_indices = []
        max_idx = len(session.results) - 1

        # Deduplicate and validate
        for idx in sorted(set(paper_indices)):
            if 0 <= idx <= max_idx:
                valid_indices.append(idx)
            else:
                logger.warning(f"Skipping out-of-range index {idx} (max: {max_idx})")

        paper_indices = valid_indices

    # Continue with validated indices...
```
...
### Riesgo 4: Cambios en formato de Google Scholar o URLs expiradas **[NUEVO REV-3]**

**Descripción**: Google Scholar puede cambiar el formato de URLs de PDFs o las URLs pueden expirar/ser bloqueadas por publishers.

**Probabilidad**: Media
**Impacto**: Medio (descargas fallan pero no rompe el sistema)

**Mitigación**:
- ✅ **Detección**: Retry logic ya maneja 403/404 errors
- ✅ **Logging**: Log completo de URL fallida + status code en `failed_papers`
- ✅ **Validación robusta**: Magic bytes %PDF asegura que incluso si Content-Type miente, rechazamos HTMLs
- ✅ **Graceful degradation**: Continuar con otros PDFs si algunos fallan
- ⚠️ **Monitoring**: En logs, trackear tasa de fallo por dominio (arxiv.org vs otros)

**Contingencia**:
- Si >50% de descargas fallan con 403: Implementar user-agent rotation (Enhanced)
- Si URLs de un dominio específico fallan sistemáticamente: Documentar en README y sugerir búsqueda manual
