...
- ⚠️ Anthropic API key configurada en `.env` (aunque no se usa en esta fase)
...
- [x] Logging a archivo (`sortgs_mcp.log`)
...
4. Session se guarda en `data/sessions/{session_id}/` con metadata.json y results.csv
5. Claude Code puede invocar el tool desde chat
6. Logs se escriben correctamente en `sortgs_mcp.log`
...
- **Logging**:
  - FileHandler → `sortgs_mcp.log` (DEBUG level para debugging completo)
  - StreamHandler → stderr (WARNING level, no contaminar stdout que es usado por MCP stdio)
...
|----|-----------|-------|-------------------------|--------------|----------|
| T1 | 🔴 CORE | Crear `server.py` con FastMCP setup y logging | - `python -m sortgs_mcp.server` arranca sin errores<br>- Logs se escriben en `sortgs_mcp.log`<br>- Stderr muestra solo warnings/errors | - | 0.5h |
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.StreamHandler()  # stderr, nivel WARNING
    ]
)

# Ajustar StreamHandler a WARNING
...
- `sortgs_mcp.log` se crea y contiene línea "Starting Sort Google Scholar MCP Server"
...
# Import MCP instance from server (evita circular import)
# Alternativa: importar FastMCP y usar decorador directo si FastMCP soporta registro global
...
**Nota importante**: FastMCP require que el decorador `@mcp.tool()` se aplique a la función. Necesitamos importar `mcp` desde `server.py` o usar un patrón de registro global.

**Solución KISS**: Definir tool en `server.py` directamente o importar `mcp` instance en `search.py`.

**Criterios de aceptación**:
- Tool registrado en MCP server
- Acepta parámetros correctos
- Retorna dict con formato esperado
- Session se guarda en filesystem
...
- No hay errores en `sortgs_mcp.log`
...
- [ ] **1.4** .env configurado (ANTHROPIC_API_KEY presente, aunque no se usa en Fase 2)
...
- [ ] **2.2** Log file creado (`ls sortgs_mcp.log` existe)
...
- [ ] **4.4** papers_found >= 1 (generalmente 10)
...
- [ ] **6.4** Archivo `results.csv` existe
- [ ] **6.5** CSV tiene headers correctos (rank, title, authors, citations, year, ...)
- [ ] **6.6** CSV contiene datos (mínimo 1 fila de paper)
...
- [ ] **7.4** Errors se loguean en `sortgs_mcp.log`
...
- [ ] **9.1** `sortgs_mcp.log` contiene logs de nivel INFO
...
1. Verificar configuración de logging:
   - FileHandler → sortgs_mcp.log (DEBUG level)
...
- ✅ **Documentación**: FastMCP es el enfoque recomendado en docs oficiales de MCP v1.2+
...
**Decisión TOMADA**:
- FileHandler → `sortgs_mcp.log`, nivel DEBUG (todo)
...
**Trade-off aceptado**: Desarrollador necesita abrir `sortgs_mcp.log` para ver logs detallados (no ve en terminal)
...
Archivo server.py:
  ...
  3. AL FINAL del archivo, importa módulos de tools: from sortgs_mcp.tools import search
  4. Define main() que llama mcp.run()

Archivo tools/search.py:
  1. Importa config, models, core components
  2. NO importa server.py directamente
