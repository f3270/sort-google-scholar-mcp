...
- [x] **Tests unitarios mínimos**: `parse_keyword_response()` con casos edge (JSON, markdown, malformado) y validación de inputs del tool

**Componentes MVP**:
- `src/sortgs_mcp/llm/openai.py`
- `src/sortgs_mcp/llm/keywords.py`
- `src/sortgs_mcp/tools/search.py` (modificación)
- `tests/test_llm_keywords.py` (parsing tests - OBLIGATORIO)
- `tests/test_tool_validation.py` (input validation - OBLIGATORIO)

### 🟡 Enhanced (Si hay tiempo)

Funcionalidades que agregan valor pero no bloquean:
- [ ] Tests unitarios exhaustivos con mocking de OpenAI (más allá de parsing básico)
...
- ❌ **Este tool NO usa SessionManager**: Es completamente stateless
- **Justificación**: Keyword generation es una operación independiente sin estado persistente
- **Diferencia con otros tools**: A diferencia de `search_papers` o `index_papers` que crean/modifican sesiones, este tool genera keywords on-demand sin side effects
- **Implicación**: No requiere session_id como parámetro, no guarda resultados en disco
- **Patrón**: Similar a un helper/utility tool - pure function que recibe input, retorna output
...
- **SDK Exceptions del módulo `openai`** (https://github.com/openai/openai-python):
  - `openai.APIError` (base exception) → Retry 3x → RuntimeError con mensaje claro
  - `openai.RateLimitError` (subclase de APIError) → Retry con backoff → RuntimeError "Rate limit exceeded"
  - `openai.AuthenticationError` → No retry → RuntimeError "Invalid API key"
  - `openai.APIConnectionError` → Retry 3x → RuntimeError "Connection failed"
- **Application Exceptions**:
  - `ValueError` (validation) → No retry → ValueError directo
  - `json.JSONDecodeError` → Intentar fallbacks de parsing → RuntimeError si todos fallan
- **Nota**: Usar `from openai import APIError, RateLimitError, AuthenticationError, APIConnectionError` para imports explícitos
...
| T9 | 🔴 CORE | **Unit tests - parsing edge cases** | Tests para `parse_keyword_response()`: JSON, markdown, malformado, vacío | T5 | 20min |
| T10 | 🔴 CORE | **Unit tests - tool validation** | Tests para validación de inputs del tool (num_variations, query vacío) | T7 | 15min |
...
- **Unit tests CORE (MVP)**: Tests de `parse_keyword_response()` con casos edge, validación de inputs del tool
...
### 9. Tests Unitarios CORE (OBLIGATORIO)
...
**Screenshot requerido**: Output de pytest mostrando tests CORE pasando
![Adjuntar aquí]
...
- [ ] ✅ **FUNCIONALIDAD COMPLETA** - Tool funciona, tests CORE pasan
...
### 10.4 Error Handling: OpenAI SDK Exceptions

**Decisión TOMADA**: Usar clases de excepción explícitas del SDK oficial `openai`

**Clases confirmadas** (openai-python v1.x):
```python
from openai import (
...
- ✅ Mensajes de error específicos por tipo (mejora UX)
...
- `AuthenticationError` → No retry, RuntimeError inmediato con hint de verificar API key
- `ValueError` (validation) → No retry, propagar directo
- Otros exceptions → Log y propagar (fail fast)

**Alternativas consideradas**:
- **Opción B (Catch genérico Exception)**: Pro: Simple, Contra: Oculta bugs, retry innecesario
- **Opción C (Solo APIError base)**: Pro: Menos imports, Contra: Pierde granularidad

**Trade-off aceptado**: Más líneas de imports pero mejor error handling

**Principio KISS aplicado**: Usar SDK oficial sin wrappers custom
...
### 10.6 Timeout Configuration: Explicit and Configurable
